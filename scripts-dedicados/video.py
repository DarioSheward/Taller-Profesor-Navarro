...
with h5py.File('./scripts-dedicados/data.h5', 'r') as f:
    ...
    def update(frame):
        ...
        theta = f['theta'][:, frame]
        p = f['p'][:, frame]
        h = ax.hist2d(theta, p, bins=100, range=[[-np.pi, np.pi], [-2, 2]], cmap='plasma', cmin=1 , cmax=10)
        ax.set_xlabel('Theta')
        ax.set_ylabel('p')
        ax.set_title(f'Phase Space Evolution at time {t[frame]:.2f}')
        return h
    ani = fa(fig, update, frames=n_frames, interval=50, blit=False, repeat=True)

    mappable = plt.cm.ScalarMappable(cmap='plasma', norm=plt.Normalize(vmin=1, vmax=20))
    mappable.set_array([])
